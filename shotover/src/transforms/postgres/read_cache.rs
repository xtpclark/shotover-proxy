...
impl Transform for PostgresReadCache {
    ...
    async fn transform<'shorter, 'longer: 'shorter>(
        ...
        for response in responses.iter_mut() {
            // Forwarded untouched, and skipped before anything parses it (see
            // `TransformConfig::accepts_partial_responses`): a partial carries no request id, so it
            ...
            // Dead work once the session has pinned: `rendering_gucs` is only ever read to build a
            // cache key, and a pinned session never reaches that.
            if !self.session_stateful {
            ...
            if let Some(rid) = response.request_id() {
                if let Some(cached) = serve_from_cache.remove(&rid) {
                    ...
                    *response = cached;
                    ...
                    // ReadyForQuery('I'), never an in-transaction train ending in 'T' (F7), never
                    // the tail of a train whose earlier rows already went out as chunks (caching that
                    // would serve a truncated, RowDescription-less row set) — AND it fits the byte
                    // budget.
                    //
                    // A streamed result reaches the chunked-tail test only if every chunk AND the
                    // tail landed in one receive, which a genuinely streamed result never does — the
                    // tail arrives in a later chain run, where `cache_on_response` is a fresh empty
                    // map. So the common case is not this branch at all: the miss is recorded and the
                    // tail is simply never matched. Both are the same per-run-local limitation; see
                    // the module doc.
                    //
...
mod tests {
    use super::{
        ...
        Relation, StmtEffect, estimate_response_size, is_pure_function, looks_volatile,
        relation_name, response_is_cacheable,
    };
    ...
    impl Transform for CannedBackend {
        ...
        async fn transform<'shorter, 'longer: 'shorter>(
            ...
            for request in &chain_state.requests {
                ...
                let mut final_chunk = encode_as(final_messages, state);
    ...
    /// THE regression: a chunked train must not be cached. Its completing chunk carries the request
    /// id and still ends in ReadyForQuery('I') with no ErrorResponse, so `response_is_cacheable`
    /// alone approves it — see `PostgresCodecState::chunked_tail`.
    #[tokio::test]
